...
<a id="2-자동-생성되는-최대-6개-대시보드-싱글톤-페이지"></a>

### 2️⃣ 자동 생성되는 최대 6개 대시보드 (싱글톤 페이지)

알림장과 함께, 데이터를 가공한 **고정 페이지(최대 6개)** 가 자동으로 만들어지고 매 실행마다 최신화됩니다. 어떤 어린이집/학원은 식단 정보가 없거나 AI를 꺼두면 일부 페이지가 생성되지 않을 수 있어요 — 모든 페이지 생성 조건은 표 아래에 정리되어 있습니다:
...
<a id="7단계-github에-3개-비밀-값-등록하기"></a>
...
| `KIDSNOTE_CHILD_NAME` | 백업할 자녀 이름 — **풀네임이든 일부든 OK**, 부분 일치 방식이라 어느 쪽이든 매칭됩니다. 예: 자녀 풀네임이 `우하린`이면 **`우하린`** 또는 **`하린`** 어느 것을 적어도 같은 결과 |
...
- 사진 OK
- 동영상/파일 첨부물 있으면 OK
- 6개 대시보드 페이지(📊 통계 / 🥗 영양 / 📖 성장 스토리 / 🌟 마일스톤 / 🌱 관심사 / 💌 선생님께)가 함께 생성됨 (식단 정보가 있고 AI=on일 때 기준; 일부만 생성될 수도 있음)
...
> 💡 **같은 DB에 두 아이를 섞으면 안 되는 이유**: 📊 통계 / 🥗 영양 / 📖 성장 스토리 등 모든 대시보드가 "한 아이 기준"으로 만들어집니다. 두 아이의 알림장이 한 DB에 들어가면 그래프가 두 아이를 합산해 의미가 사라져요.
...
>
> **자녀 선택도 같은 방식** — 백업할 아이를 고정하는 secret은 **`KIDSNOTE_CHILD_NAME`**. 값은 풀네임이든 일부든 OK (자녀 풀네임이 `우하린`이라면 **`우하린`** 또는 **`하린`** 어느 것을 적어도 같은 결과). 자녀가 1명이어도 꼭 등록하세요 — API 자동 선택에 의존하지 않고 동생이 생겨도 안전합니다. 자녀가 여러 명인 경우에는 [👨‍👩‍👧‍👦 자녀가 여러 명이거나 AI 편지를 끄고 싶을 때](#advanced-multichild-and-ai-off) 섹션 참고.
...
> 💡 **왜 통계 페이지가 따로 모이나요?** 통계·AI 대시보드 6개는 이름이 `_`로 시작합니다 (ASCII `_` < `[` 라서 내림차순 정렬 시 위로 모임). 알림장은 `[YYYY-MM-DD]`로 시작해서 그 아래에 시간 역순으로 정렬됩니다. 통계 페이지가 알림장 사이에 끼어들지 않아 깔끔합니다.
...
<a id="ai가-만드는-편지에-자녀-이름이-이상해요-예-유주이야-로-부르고-있어요"></a>
...
- **Ollama** on the runner (`llama3.1:8b`, CPU-only) — 6개 대시보드 중 LLM 4개 생성에 사용. 모델·바이너리는 `actions/cache@v4`로 영구 캐싱

**핵심 코드** ([`tools/kidsnote_fetch/`](tools/kidsnote_fetch/), 약 4000줄):
- `fetch.py` — 키즈노트 API 호출 + 메인 로직 + dashboard 라우팅
- `notion_mirror.py` — 노션 DB 쓰기 + 이미지 압축 + EXIF strip + 6개 dashboard 생성 + LLM 호출 + 한국어 호칭 헬퍼(`_addressee`/`_vocative_marker`)
