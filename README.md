...
- [⭐ 시작하기 — 8단계](#시작하기)
  ...
  - [6단계. 키즈노트 아이디·비밀번호](#6단계-키즈노트-아이디-비밀번호-확인하기)
  - [7단계. GitHub Secrets](#7단계-github에-6개-비밀-값-등록하기)
...
> 💡 **권장: Public fork** — GitHub Actions 무제한이라 AI=on 1년치 5-15시간 백업도 무관. 개인정보(비밀번호·토큰)는 모두 GitHub Secrets에 암호화되어 코드 공개와 상관없이 안전합니다.

<a id="4-시간-셋업-25분-백업은-자동"></a>

### 4. 시간 — 셋업 25분 / 백업은 자동

- **사용자가 직접 손대는 시간**: 셋업 25분 (이후 할 일 없음)
...
각 알림장은 **노션 페이지 한 장**으로 백업됩니다 — 원본 본문 + 사진(원본 화질·GPS 자동 제거) + 식단 사진 + 댓글 + 날씨. AI를 켜면 자녀 1인칭 일기·부모 편지도 함께 (셋업 7단계에서 선택).
...
> - **Public** (기본): GitHub Actions 무제한 무료 사용 가능. **개인정보(비밀번호·토큰)는 Secrets에 분리 저장**되어 코드 공개와 무관하게 안전.
...
<a id="6단계-키즈노트-아이디-비밀번호-확인하기"></a>

## 6단계. 키즈노트 아이디·비밀번호 확인하기

> 🕐 **약 1분**
>
> 📍 **시작 전 확인**: 키즈노트 웹 로그인 확인 완료 (0단계)

**따로 할 일은 없습니다.** https://www.kidsnote.com 에 로그인할 때 쓰는 **아이디와 비밀번호**를 7단계에서 그대로 입력합니다.

> 💡 비밀번호가 기억나지 않으면 키즈노트에서 비밀번호를 재설정한 뒤 새 비밀번호를 쓰세요.

✅ **6단계 성공 신호**: https://www.kidsnote.com 에서 그 아이디·비밀번호로 로그인이 됨.

---

<a id="7단계-github에-6개-비밀-값-등록하기"></a>

## 7단계. GitHub에 6개 비밀 값 등록하기

> 🕐 **약 5분**
>
> 📍 **시작 전 확인**: 메모장에 두 값(노션 토큰, DB ID)이 있고, 키즈노트 아이디·비밀번호를 알고 있고, GitHub fork 페이지를 찾을 수 있음 (1단계 URL)

이제 메모장에 모은 값과 키즈노트 아이디·비밀번호를 GitHub fork에 저장합니다.
...
✅ **7단계 성공 신호**: Secrets 목록에 다음 6개가 정확한 이름으로 나열됨:
```
...
| 시크릿이 7개 이상 있음 | 이전에 다른 시크릿 만든 적 있음. 위 6개만 정확하면 추가 시크릿은 무시 가능 |
...
>
> 📍 **시작 전 확인**: 시크릿 6개 등록 완료 + 노션 DB에 통합 연결 완료
...
키즈노트 계정에 자녀가 N명 (쌍둥이·형제·자매)이면, 각 자녀를 **별도 GitHub fork에서 병렬 백업**하는 것이 가장 빠릅니다. GitHub Actions runner는 fork별로 따로 돌기 때문에, 자녀 N명을 같은 시간에 동시 백업할 수 있어요.
...
| Fork 시크릿 `KIDSNOTE_USERNAME` / `KIDSNOTE_PASSWORD` | 키즈노트 아이디 / 비밀번호 | 자녀 #1과 같은 값 |
| Fork 시크릿 `KIDSNOTE_CHILD_NAME` | 첫째 이름 (예: `하린`) | 둘째 이름 (예: `혁주`) |

### 📋 자녀 #2 셋업 단계 (자녀 #1 기준 셋업이 이미 끝났다고 가정)

자녀 #1 셋업 + **GitHub fork와 노션 쪽을 한 번 더** 만듭니다. 키즈노트 계정이 같으면 아이디·비밀번호도 자녀 #1과 같은 값을 씁니다.
...
#### Step 6. 자녀 #2 fork에 시크릿 6개 등록
자녀 #2용 fork에서 [**7단계**](#7단계-github에-6개-비밀-값-등록하기) 그대로 진행:

| Name | 값 |
|---|---|
| `NOTION_TOKEN` | 자녀 #2 통합 토큰 (Step 2) |
| `NOTION_DATABASE_ID` | 자녀 #2 DB ID (Step 4) |
| `KIDSNOTE_USERNAME` | **자녀 #1 fork와 같은 아이디** |
| `KIDSNOTE_PASSWORD` | **자녀 #1 fork와 같은 비밀번호** |
...
| GitHub ❌ 메일이 오면 | [키즈노트 로그인 실패 메일이 왔을 때](#키즈노트-로그인-실패-메일이-왔을-때) 또는 [문제 해결](#문제-해결-자주-나오는-에러) 참고 |
...
### 키즈노트 로그인 실패 메일이 왔을 때

fork repo → **`Actions`** 탭 → 빨간색 ❌ run을 클릭하면 **`키즈노트 로그인 실패`** 와 그 이유가 한국어로 표시됩니다.

> ⚠️ 이 메일은 **처음 한 번만** 옵니다. 그 뒤 자동 실행은 경고만 남기고 녹색으로 끝나며, 고칠 때까지 백업은 건너뜁니다.

**확인할 것**:
- **키즈노트 비밀번호를 바꿨다** → `KIDSNOTE_PASSWORD`를 새 비밀번호로 수정
- **아이디·비밀번호나 시크릿 이름에 오타가 있다** → [7-3](#7-3-시크릿-6개-등록) 표와 똑같은지 확인 후 다시 입력
- **2단계 인증을 켰다** → [(부록) 2단계 인증을 켜둔 키즈노트 계정](#부록-2단계-인증을-켜둔-키즈노트-계정)
- **예전에 쿠키 방식으로 셋업했다** → [7-3](#7-3-시크릿-6개-등록)의 `KIDSNOTE_USERNAME`, `KIDSNOTE_PASSWORD` 두 시크릿을 새로 추가

**시크릿 수정 방법**:
1. fork repo → **`Settings`** → **`Secrets and variables`** → **`Actions`**
2. 고칠 시크릿 옆 연필 아이콘 클릭 → 값 다시 입력 → **`Update secret`**

고치고 나면 **다음 자동 실행(6시간마다) 때 백업이 알아서 다시 시작**됩니다. 바로 재개하려면 위 **수동 trigger** 순서대로 실행하세요.
...
자녀가 여러 명인 키즈노트 계정은 기본적으로 **API가 첫 번째로 반환한 자녀**가 백업됩니다.
...
1. **AI 통째로 끄기** — repo Settings에서 `AI_FEATURES` 값을 `off`로 변경 → 단순 백업 + 통계만
...
https://www.kidsnote.com 로그인 화면에서 **아이디·비밀번호로 로그인되는 계정이면 됩니다**. 그 값을 [7단계](#7-3-시크릿-6개-등록)에 등록하세요.

<a id="매번-수동으로-실행해야-하나요"></a>

### 매번 수동으로 실행해야 하나요?

**아니요 — 한 번만 셋업하면 끝입니다.** 6시간마다 자동으로 cron이 새 알림장을 백업합니다.
...
먼저 **키즈노트 로그인 실패**인지 확인하세요:

1. fork repo → `Actions` 탭 → 최근 run 클릭 → **`키즈노트 로그인 실패`** 표시가 있는지 보기 (녹색 run이어도 확인)
2. 있으면 [키즈노트 로그인 실패 메일이 왔을 때](#키즈노트-로그인-실패-메일이-왔을-때) 절차
3. 없는데 안 들어오면 오늘 어린이집 휴무일이라 새 글이 없을 수도

<a id="내-데이터가-안전한가요"></a>

### 내 데이터가 안전한가요?

- 비밀값(비밀번호·토큰)은 GitHub Secrets로 암호화 저장
...
| 에러 메시지 | 원인 | 해결 |
|---|---|---|
| `NOTION_TOKEN / NOTION_DATABASE_ID missing` | 시크릿 이름 오타 또는 누락 | 7단계 시크릿 이름 정확히 (대문자+언더바) |
| **키즈노트 로그인 실패** | 아이디·비밀번호 불일치 또는 2단계 인증 | [키즈노트 로그인 실패 메일이 왔을 때](#키즈노트-로그인-실패-메일이-왔을-때) |
...
| **📖 / 🌟 / 🌱 / 💌 AI 페이지 안 보임** | `AI_FEATURES=off`이거나, 개별 `GROWTH_STORY`/`MILESTONES`/`INTERESTS`/`TEACHER_THANKS` 중 하나가 `off`로 설정됨 | repo Settings에서 `AI_FEATURES` 값을 `on`으로 변경. 개별 disable secret이 있으면 삭제하거나 값을 `on`으로 |
...
이미 등록한 `KIDSNOTE_USERNAME`·`KIDSNOTE_PASSWORD`는 그대로 둬도 됩니다. 쿠키가 만료되면 [로그인 실패 메일](#키즈노트-로그인-실패-메일이-왔을-때)이 옵니다.
