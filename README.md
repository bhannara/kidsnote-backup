...
> - **AI 켰는데 특정 페이지만 빼고 싶음**: 같은 화면에 `GROWTH_STORY` / `MILESTONES` / `INTERESTS` / `TEACHER_THANKS` 중 원하는 것을 값 `off`로 추가. 자세한 가이드는 [B. AI 가공을 안 쓰고 단순 백업 + 통계만 원할 때](#b-ai-가공을-안-쓰고-단순-백업-통계만-원할-때) 섹션 참고.
...
> ⏱️ 첫 실행은 LLM 모델 다운로드 때문에 15-20분 걸립니다. 두 번째 실행부터는 캐싱되어 ~30초 만에 시작.
