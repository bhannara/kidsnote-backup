...
- ✅ **최대 7개 대시보드 자동 생성** — 매월 성장 스토리, 마일스톤, 분기별 관심사, 연도별 선생님께 등 (LLM이 자동 작성. 데이터에 따라 6~7개)
...
- [(고급) 자녀가 둘 이상이거나 AI 편지를 끄고 싶을 때](#-고급-자녀가-여러-명이거나-ai-편지를-끄고-싶을-때)
...
각 알림장 페이지는 **원본 본문 + AI 가공 토글(3개) + 날씨 callout(원본 데이터)**으로 구성됩니다:

```
[2026-03-31] 알림장: 👩‍🏫 🎵 음악 · 🌸 꽃 · 🧩 블록
👩‍🏫 새싹2반 교사 · 새싹2반 · 작성 2026-03-31 · 🍽️ 정해진 식단 · 💤 1~1.5시간 ...

▶ 💭 본문 요약 (AI 가공 — 펼치기)            ← 디폴트 접힘, 클릭 시 펼침
▶ 🧒 자녀의 일기 (AI 가공 — 펼치기)
▶ 👨‍👩‍👧 부모의 편지 (AI 가공 — 펼치기)
🌤️ 오늘의 날씨: ⛅ 흐림                       ← 키즈노트 원본, 토글 아님
...
> 💡 **AI 가공 콘텐츠는 디폴트로 숨겨져 있습니다.** 페이지를 열면 원본 알림장이 먼저 보이고, AI 결과는 ▶ 토글을 클릭해서 펼쳐야 노출됩니다. 마음에 들면 펼쳐서 읽고, 안 들면 그냥 무시. 자녀 이름의 호격(`유주야`/`하린아`)은 받침을 자동 인식해서 정확히 처리합니다.

펼쳤을 때 안에 들어있는 내용은:
- **💭 본문 요약** (purple, 한 줄): kiwipiepy 형태소 분석 보조 + LLM 한 줄 요약
- **🧒 자녀의 일기** (yellow, 2-3문장): 알림장을 자녀의 1인칭 시점으로 변환 (LLM)
- **👨‍👩‍👧 부모의 편지** (pink, 2-3문장): 자녀가 자라서 봤을 때 감동받을 짧은 편지 (LLM)

토글 없이 항상 보이는 것:
- **🌤️ 오늘의 날씨**: 키즈노트에서 선생님이 직접 입력한 값 그대로 (LLM 추정 없음)

### 🛡️ AI 품질 보호 장치 (자동 적용)

LLM이 가끔 헛소리를 만드는 걸 막기 위해 코드 안에 세 가지 자동 안전장치가 들어있습니다 — 사용자가 따로 설정할 거 없음:

1. **사건 anchoring** — 자녀 일기·부모 편지를 만들기 전에 알림장 본문에서 명사구를 추출해 LLM에 "이 사실들만 사용해라" 명시. 본문에 없던 사건/장소/사람이 추가되는 환각이 크게 줄어듭니다.

2. **영어 학습 알림장 자동 인식** — 어학원/영어 학습 알림장(본문에 영어 알파벳 25% 이상)은 1인칭 일기·부모 편지로 변환하기 어색합니다. 이런 경우 두 토글은 자동으로 "AI 가공 생략" placeholder가 표시되고, 💭 한 줄 요약만 한국어로 생성됩니다.

3. **외국어/한자 leak 자동 정규화** — LLM이 가끔 끼워넣는 일본어 `。、！？` 같은 fullwidth 구두점은 자동으로 한국어 `.,!?`로 치환. 중국어 한자는 비율 20% 이상이면 그 출력 자체를 reject + placeholder.
...
### 2️⃣ 자동 생성되는 최대 7개 대시보드 (싱글톤 페이지)

알림장과 함께, 데이터를 가공한 **고정 페이지(최대 7개)** 가 자동으로 만들어지고 매 실행마다 최신화됩니다. 어떤 어린이집/학원은 식단 정보가 없거나 AI를 꺼두면 일부 페이지가 생성되지 않을 수 있어요 — 모든 페이지 생성 조건은 표 아래에 정리되어 있습니다:

| 페이지 | 내용 |
|--------|------|
| 📊 **통계 대시보드** | 누계 카테고리·월별·작성자·낮잠·식사·날씨 분포 + **연도별 상세 sections** (각 연도의 알림장 수, 카테고리·월별 분포 등) |
| 📅 **작년추억** | 오늘 날짜와 같은 작년·재작년 같은 날의 알림장을 모아 보기 |
...
> 👨‍👩‍👧‍👦 **자녀가 2명 이상인가요?** 아래 1~8단계는 **자녀 1명 기준**입니다. 둘째 아이는 셋업을 마친 뒤 자녀별로 노션 DB·통합·시크릿을 따로 만들어야 합니다 — 자세한 가이드는 [(고급) 자녀가 둘 이상이거나 AI 편지를 끄고 싶을 때](#-고급-자녀가-여러-명이거나-ai-편지를-끄고-싶을-때) 섹션 참고.
...
9. 다시 표 우측 끝 **`+`** 클릭 → 또 새 속성 추가:
   ...
   - **그 다음** 이름 입력란에 `Report ID` 입력 ⚠️ **정확히 영문 대소문자로. R은 대문자, ID는 대문자**

✅ **2단계 성공 신호**: 표 머리글에 기본 속성 두 개 + **`날짜`** + **`Report ID`** 컬럼이 보임. 표 본문(행)은 비어 있어도 OK.
...
| Report ID에 자동 띄어쓰기/한글 변환 | 한국어 IME가 자동 변환. 유형(`숫자`)을 먼저 고른 다음 이름 입력란에서 한/영 키 다시 누른 후 재입력 |
...
### 5-1. DB 페이지를 풀화면으로 열기 (권장 — 자동 복구는 되지만 깔끔하게)

> 💡 **부모 페이지 ID를 잘못 넣어도 코드가 자동으로 진짜 DB ID를 찾아내고 한 줄 안내 후 백업을 계속 진행합니다.** (`NOTION_DATABASE_ID ... looks like a parent page id ... Auto-recovered Notion DB id from parent page`) 그래도 정확한 DB ID를 secret에 직접 등록하는 게 깔끔하니, 아래 풀화면 단계를 권장합니다.
...
>
> GitHub-hosted runner는 단일 job **6시간 하드 캡**이 있어서 AI 켬 + 대용량은 한 번에 못 끝낼 수 있는데, 4시간 cron이 자동 재개해서 사용자가 다시 클릭할 일은 없어요.
...
2. **7개 대시보드 자동 생성** — 통계/작년 추억/영양 + 4개 LLM 페이지 (성장 스토리/마일스톤/관심사/연도별 선생님께). 새 알림장이 publish되면 LLM 대시보드도 자동 갱신
...
- 사진 클릭하면 원본 화질로 열림
- DB 안에 자동 생성된 **7개 대시보드** 페이지도 보임 (📊 통계 / 📅 작년 추억 / 🥗 영양 / 📖 성장 스토리 / 🌟 마일스톤 / 🌱 관심사 / 💌 선생님께)
...
[**3단계**](#3단계-노션-통합integration-만들기--토큰-받기)를 한 번 더 합니다. 통합 이름은 `kidsnote-backup-child2` 같이 구분되게 지으세요. **새 토큰**(`ntn_...`)이 발급됩니다 — 이게 자녀 #2 전용 토큰입니다.
...
졸업·휴원 후 백업이거나, 사생활을 더 보수적으로 다루고 싶거나, AI가 만든 글이 어색하다고 느끼면 **클릭 한 번으로 전부 끄고 단순 백업만** 받을 수 있어요. 통계 대시보드 3개 (📊 통계 / 📅 작년 추억 / 🥗 영양)는 LLM 없이 만들어지므로 그대로 생성됩니다.
...
- 워크플로가 Ollama 다운로드까지 스킵해서 **5분 정도 빨리 끝남**
...
> - 4시간 cron 자동 백업은 본인 노션 DB에만 쓰므로 외부 노출 없음
...
> 💡 **왜 통계 페이지가 따로 모이나요?** 통계·AI 대시보드 7개는 이름이 `_`로 시작합니다 (ASCII `_` < `[` 라서 내림차순 정렬 시 위로 모임). 알림장은 `[YYYY-MM-DD]`로 시작해서 그 아래에 시간 역순으로 정렬됩니다. 통계 페이지가 알림장 사이에 끼어들지 않아 깔끔합니다.
...
> 💡 **왜 콜론까지 포함?** 알림장 본문에 "알림장에서 ..." 같은 표현이 있을 수 있어, 콜론(`:`)을 포함해야 제목 prefix(`[2024-10-19] 알림장: ...`)만 정확히 매칭됩니다.
...
| 📊 통계만 | 이름 starts with `_` | 이름 오름차순 | 7개 대시보드만 |
...
다른 자녀를 백업하려면 `Run workflow` 화면의 **`child_name`** 칸에 자녀 이름을 입력하세요 (예: `유주`). 자세한 가이드는 [👨‍👩‍👧‍👦 (고급) 자녀가 둘 이상이거나 AI 편지를 끄고 싶을 때](#-고급-자녀가-여러-명이거나-ai-편지를-끄고-싶을-때) 섹션을 참고하세요.

> 💡 **자녀가 정확히 어떤 이름·ID로 등록돼 있는지 확인**: 워크플로를 한 번 돌리면 시작 직후 로그에 `Account has N child(ren): #1 id=... name=...` 형태로 전체 목록이 출력됩니다.

### AI가 만드는 편지에 자녀 이름이 이상해요 (예: "유주이야"로 부르고 있어요)

이전 버전에서 `_topic_form` 헬퍼가 빠져 모음으로 끝나는 자녀 이름(`유주`, `지우` 등)에 `이`가 잘못 붙던 문제가 있었습니다 (`유주이가` → 호격 `유주이야`). **2026-05-21 이후 버전부터는 받침을 자동 인식**하므로 정상 출력됩니다 (`유주가` → `유주야`).
...
자세한 가이드는 [👨‍👩‍👧‍👦 (고급) 자녀가 둘 이상이거나 AI 편지를 끄고 싶을 때](#-고급-자녀가-여러-명이거나-ai-편지를-끄고-싶을-때) 섹션 참고.
...
| **다른 자녀가 백업됨** (둘째인데 첫째가 백업됨) | 자녀가 둘 이상이면 API상 첫 번째 자녀가 default. `child_name` workflow input 누락 | `Run workflow` 시 **`child_name`** 칸에 백업하려는 자녀 이름 일부 입력 (예: `유주`). 자세히는 [(고급) 자녀가 둘 이상이거나 AI 편지를 끄고 싶을 때](#-고급-자녀가-여러-명이거나-ai-편지를-끄고-싶을-때) 섹션 |
