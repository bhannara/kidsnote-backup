...
### 5-1. DB 페이지를 풀화면으로 열기 (⚠️ 가장 흔한 함정)

> 🚨 **이 단계를 건너뛰면 워크플로 실행 시 `400 Bad Request — is a page, not a database` 에러가 납니다.** 인라인 DB는 부모 페이지 안에 들어 있어서 그 페이지의 URL을 그대로 복사하면 **부모 페이지 ID**가 잡혀요. 우리에게 필요한 건 **DB 블록 자체의 ID**입니다.
...
| 워크플로 실행 시 `400` 에러 + `is a page, not a database` 메시지 | **부모 페이지 ID를 잘못 등록한 경우.** URL에 `?v=`가 빠져있으면 페이지 URL인 거예요. 5-1로 돌아가서 ↗ 풀화면 클릭 후 다시 ID 추출 |
...
> 💡 **같은 DB에 두 아이를 섞으면 안 되는 이유**: 📊 통계 / 📅 추억 / 🥗 영양 / 📖 성장 스토리 등 모든 대시보드가 "한 아이 기준"으로 만들어집니다. 두 아이의 알림장이 한 DB에 들어가면 그래프가 두 아이를 합산해 의미가 사라져요.
...
2. **별도 페이지로** 4개 AI 대시보드 (📖 매월 성장 스토리 · 🌟 마일스톤 · 🌱 분기 관심사 · 💌 선생님께 감사 카드)

졸업·휴원 후 백업이거나, 사생활을 더 보수적으로 다루고 싶거나, AI가 만든 글이 어색하다고 느끼면 **클릭 한 번으로 전부 끄고 단순 백업만** 받을 수 있어요. 통계 대시보드 3개 (📊 통계 / 📅 추억 / 🥗 영양)는 LLM 없이 만들어지므로 그대로 생성됩니다.
...
| `disable_teacher_thanks` | 💌 선생님께 감사 카드 | LLM이 쓰는 졸업·연말 감사 편지 초안 |
...
_ 📊 2026년 통계 대시보드          ┐
_ 📅 2026년 오늘의 추억             │
_ 🥗 2026년 영양 분석              │ 7개의 통계·AI 대시보드가
_ 📖 2026년 매월 성장 스토리        │ 맨 위에 모입니다
_ 🌟 2026년 우리 아이의 처음들      │ (이름이 `_`로 시작하기 때문)
_ 🌱 2026년 분기별 관심사           │
_ 💌 2026년 선생님께                ┘
[2026-05-21] 알림장: ...           ┐
[2026-05-20] 알림장: ...           │
